...
# ── BASE DIRECTORY (works everywhere!) ───────────
BASE_DIR = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))

# ── FLASK APP ─────────────────────────────────────
app = Flask(__name__, 
    template_folder=os.path.join(BASE_DIR, 'Frontend', 'templates'))
app.secret_key = os.environ.get('SECRET_KEY', 'burnoutiq_secret_2024')
...
def predict():
    ...
    cursor.execute("""
        INSERT INTO predictions (
            user_id, anxiety_level, self_esteem, mental_health_history,
            depression, sleep_quality, academic_performance, study_load,
            future_career_concerns, social_support, peer_pressure, stress_level
        ) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)
    """, (
        session['user_id'],
